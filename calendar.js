/* ==========================================================
   BlueSite — Admin Calendar (Local Storage Until Checkout)
========================================================== */
...
function renderWeek(date) {
    ...
    updateCheckout(); // refresh checkout list after week render
}
...
function enableInteractions(col) {
    ...
    col.addEventListener("mouseup", e => {
        ...
        // Delete overlapping selections first
        ...
        // Save locally and update checkout
    ...
    for (const date in userSelections) {
        for (const sel of userSelections[date]) {
            ...
            removeBtn.addEventListener("click", () => {
                // Remove from userSelections and DOM
                sel.element.remove(); // remove from calendar
                userSelections[date] = userSelections[date].filter(s => s !== sel);
                updateCheckout(); // refresh checkout list
            });
...
if (checkoutBtn) {
    checkoutBtn.addEventListener("click", async () => {
        ...
        if (!payload.length) {
            alert("No bookings to save."); // popup if empty
            return;
        }
        ...
        try {
            ...
            // popup on success or failure returned from server
            if (data.success) {
            ...
            // Clear local selections and refresh seamlessly
            userSelections = {};
